...
const AdminNotificationSystem = () => {
  ...
  return <div className="relative">
      <Button variant="outline" size="icon" onClick={() => setIsOpen(!isOpen)} className="relative bg-inherit text-primary-foreground">
      ...
      {isOpen && <Card className="absolute top-12 right-0 w-96 z-50 shadow-lg border">
          ...
          <CardContent className="p-0">
            <ScrollArea className="h-96">
              {notifications.length === 0 ? <div className="p-4 text-center text-muted-foreground">
                  ...
                  {notifications.map(notification => <div key={notification.id} className={`p-3 border-b last:border-b-0 hover:bg-muted/50 ${!notification.read ? 'bg-primary/5' : ''}`}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {(notification.device || notification.location) && <div className="text-xs text-muted-foreground space-y-1">
                              {notification.device && <div className="flex items-center gap-1">
                                  <Monitor className="h-3 w-3" />
                                  <span>{notification.device}</span>
                                </div>}
                              {notification.location && <div className="flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            {!notification.read && <Button variant="ghost" size="sm" onClick={() => markAsRead(notification.id)} className="h-6 px-2 text-xs">
                                Mark read
                              </Button>}
                            <Button variant="ghost" size="sm" onClick={() => deleteNotification(notification.id)} className="h-6 px-2 text-xs text-destructive hover:text-destructive">
                              Delete
                            </Button>
                          </div>
                        </div>
